...
def compute_firing_rates(directory: str, params: dict) -> Tuple[float, float]:
    ...
    for stim in range(n_stimuli):
        ...
        exc_counts.append(spikes[:n_excite].var(1) / spikes[:n_excite].mean(1))
        inh_counts.append(spikes[n_excite:].var(1) / spikes[n_excite:].mean(1))
    ...
    mean_exc_rate = np.mean(all_exc_counts)
    mean_inh_rate = np.mean(all_inh_counts)
...
def plot_heatmaps(results: dict, fig_dir: str = "figures"):
    ...
    metrics = [
        ('exc_fano', 'Excitatory Fano Factor', 'YlOrRd'),
        ('inh_fano', 'Inhibitory Fano Factor', 'YlGnBu'),
    ]
    ...
    plt.suptitle(f'Fano Factors: {param1_name} vs {param2_name}', fontsize=14)
    plt.tight_layout()

    combined_path = fig_path / 'fano_factor_heatmaps.png'
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate heatmaps of fano factors across parameter sweep'
    )
    ...
    if n_completed > 0:
        ...
        # Print summary
        print("\n" + "="*60)
        print("SUMMARY: Fano factors across completed parameter points")
        print("="*60)
        for key, label in [('exc_fano', 'Excitatory'), ('inh_fano', 'Inhibitory')]:
            data = results[key]
